...
# Determine the download tag.  
# - Experimental tags look like: cdda-experimental-YYYY-MM-DD-HHMM  
#   (e.g. cdda-experimental-2026-08-12-0250). These must pass through  
#   VERBATIM - do NOT strip anything and do NOT run the stable base-tag logic.  
# - Stable "iteration" tags look like: 0.I or 0.I-build2. For those we strip  
#   the -build* suffix to get the base tag (e.g. 0.I-build2 -> 0.I).  
if [[ "$RELEASE_TAG" == cdda-experimental-* ]]; then  
  echo "Detected experimental release tag."  
  BASE_TAG="$RELEASE_TAG"  
elif [ "$RELEASE_TAG" = "latest-experimental" ]; then  
  echo "Fetching latest experimental release..."  
  # Newest experimental tag by version sort.  
  LATEST_TAG=$(git ls-remote --tags https://github.com/${CDDA_REPO}.git | grep -E 'refs/tags/cdda-experimental-' | sed 's/.*\///' | sort -V | tail -n1)  
  echo "Latest experimental tag: $LATEST_TAG"  
  BASE_TAG="$LATEST_TAG"  
else  
  # Stable path: extract base tag from iteration tags (e.g. 0.I-build2 -> 0.I)  
